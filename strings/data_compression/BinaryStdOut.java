 ...
 *  one 8-bit char, one 32-bit int, one 64-bit long, one 32-bit float,
 *  or one 64-bit double at a time.
 *
...
public class BinaryStdOut {
    ...
    private static boolean initialized; // has BinaryStdOut been called for first time?
    ...
    /**
     * write out any remaining bits in buffer as byte to standard output, padding with 0s.
     */
    ...
    public static void write(int x, int r) {
        ...
        if (r < 1 || r > 32) {
            throw new IllegalArgumentException(String.format("Illegal value for r = %d", r));
        }
        if (x < 0 || x >= (1 << r)) {
    ...
    public static void write(char x) {
        if (x < 0 || x >= 256) {
            throw new IllegalArgumentException(String.format("Illegal 8-bit char = %d", x));
        }
        writeByte(x);
    }
