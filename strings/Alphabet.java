...
public class Alphabet {
    ...
    public Alphabet(String alpha) {
        ...
        for (int i = 0; i < alpha.length(); i++) {
            ...
            if (unicode[c])
                throw new IllegalArgumentException(String.format("Illegal alphabet: repeated character = '%c'", c));
            unicode[c] = true;
        }
    ...
    public char toChar(int index) {
        if (index < 0 || index >= R) {
            throw new IllegalArgumentException(String.format("index must be between 0 and %d: %d", R, index));
        }
