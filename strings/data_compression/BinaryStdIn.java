 ...
 *  32 bits at a time (as an {@code int} or {@code float}),
 *  or 64 bits at a time (as a {@code double} or {@code long}).
 *
 *  All primitive types are assumed to be represented using their
 *  standard Java representations, in big-endian (most significant
 *  byte first) order.
 *
 *  The client should not intermix calls to {@code BinaryStdIn} with calls to {@code StdIn} or {@code System.in};
...
public final class BinaryStdIn {
    ...
    private static int buffer; // a byte buffer
    private static int n; // remaining bits in buffer
    private static boolean initialized; // has BinaryStdIn been called for first time?
     ...
     * @return the next bit of data from standard input ad a {@code boolean}, true it bit is 1, else false.
     ...
     * to read the next 16 buts as a char, use {@code readChar(16)}
     *
    ...
    public static char readChar() {
        ...
        // combine last n bits of current buffer with first 8 - n bits from new buffer.
        int x = buffer;
        x = x << (8 - n);
        int oldN = n;
        fillBuffer();
    ...
    /**
     * Read the next r bits from standard input and returns as an r-bit character.
    ...
    public static char readChar(int r) {
        if (r < 1 || r > 16) {
            throw new IllegalArgumentException(String.format("Illegal value of r = %d", r));
        }

        // optimize for r = 8 case
        if (r == 8) {
            return readChar();
        }
        char x = 0;
